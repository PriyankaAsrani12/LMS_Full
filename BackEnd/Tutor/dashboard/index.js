...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const second = [];
    const finalSql2 = `
     SELECT  DATE(createdAt) as  label, COUNT(student_id) as  totalCount FROM  student_purchases WHERE  createdAt >= date_sub(curdate(), interval 6 day) AND customer_id=${req.user.customer_id}  GROUP   BY  DATE(createdAt);
     SELECT DISTINCT COUNT(student_id) as totalCount1,YEAR(createdAt) as year,WEEK(createdAt) AS weekNo FROM student_purchases  WHERE createdAt >= (NOW() - INTERVAL 1 MONTH) AND customer_id=${req.user.customer_id} GROUP BY weekNo;
     SELECT YEAR(createdAt) AS y, MONTH(createdAt) AS m, COUNT(student_id) as totalCount2 FROM student_purchases wHERE createdAt >= (NOW()- INTERVAL 3 MONTH) AND customer_id=${req.user.customer_id} GROUP BY y, m;
     `;
